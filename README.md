...
As demonstrated above, the application uses a simple versioning scheme for all components:

  `<git-commit-timestamp>.<git-commit-short-hash>`

The versioning scheme also appends a build identifier if the `BUILD_ID` environment variable is set:
...
...
...
$ gradle :microtrader-audit:clean :microtrader-audit:test
...
...
```
...
- Application services - the various application services are built into "release" images as defined in `Dockerfile.<service>` files in the [docker/release](./docker/release) folder.  These images represent the final artifact that will run in production, assuming all future tests and quality checks pass.  Each application service release image is based from the [`microtrader-base`](https://github.com/dpaws/microtrader-base) image.
...
Step 1 : FROM dpaws/microtrader-base
...
...
...
$ make publish
=> Publishing development image sha256:fb00a7fb8df435b8189604f761c3e30efceece6f58e67acc5779ee117dd1c623 to docker.io/dpaws/microtrader-dev...
The push refers to a repository [docker.io/dpaws/microtrader-dev]
...
...
...
=> Publishing release images to docker.io/dpaws...
The push refers to a repository [docker.io/dpaws/microtrader-quote]
...
...
```
...
- `make test`
- `make release`
- `make tag` - tags the current version (i.e. `make version`), short commit hash, any annotated tags that are present on the current commit and the 'latest' tag
- `make publish`
- `make clean` 
...
| CLUSTER_PUBLIC_ADDRESS |                 | Sets the public IP address of the instance in a NAT scenario.  In a Docker environment, you would need to set this environment variable to the host IP address.  You can set the CLUSTER_AWS_DETECTION variable to true in an AWS environment to auto-populate this setting.               |
| CLUSTER_PUBLIC_PORT    | 5701            | Sets the public port of the instance in a NAT scenario.  In a Docker environment, this needs to match the host port mapped to the container port for Hazelcast clustering.                                                                                                                 |
| CLUSTER_PORT           | 5701            | Sets the private port of the instance.  In a Docker environment, this sets the container port used for Hazelcast clustering                                                                                                                                                                |
| CLUSTER_AWS_DETECTION  | False           | When set to "true", queries the EC2 metadata service to determine the host IP address.                                                                                                                                                                                                     |
